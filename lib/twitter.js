...
Twitter.prototype.__buildEndpoint = function(path, base, params) {
  var bases = {
  ...
  endpoint = endpoint.replace(/:(\w+)/g, function(match) {
    var param = match.slice(1);
    var val = params[param].toString();
    if (!val) {
      throw new Error('This parameter has no value: ' + param);
    }
    delete params[param];
    return val;
  });
...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  var url = this.__buildEndpoint(method, base, params);
